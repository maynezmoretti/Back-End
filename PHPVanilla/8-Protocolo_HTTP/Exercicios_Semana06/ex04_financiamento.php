...
// Exercício 4: Simulador de Financiamento de Veículos (POST)

// Requisitos:
// - Campos:
// valorVeiculo (R),
// valorEntrada(R)
// numeroParcelas (select com opções: 12, 24, 36, 48, 60).
...
const JUROS = 0.015; // 1,5% a.m.
const PERCENTUAL_MINIMO_ENTRADA = 0.20; // 20% de entrada mínima
...
if($_SERVER["REQUEST_METHOD"] === "POST"){
    ...
    $numeroParcelas = filter_var($_POST["numeroParcelas"] ?? 0, FILTER_VALIDATE_INT);
    ...
    if($valorVeiculo === false || $valorVeiculo <= 0){
        ...
        $erros[] = "A entrada deve ser de pelo menos 20% , R$ {$entradaMinima}";
    }

    // Validação 2 - Número de parcelas
    ...
    if(empty($erros)){ // se não existir erros
        $saldoFinanciado = $valorVeiculo - $valorEntrada;

        // Cálculo de Juros Simples -> se fosse Juros Compostos, usaríamos um laço de repetição (for)
